...
class ServerUDP_TCP:
    ...
    def get_name_by_address(self, client_address: tuple) -> Optional[str]:
        with self.registration_lock:
            for name, info in self.registration_data.items():
                if info['ip'] == client_address[0] and info['udp_port'] == client_address[1]:
                    return name
        return None
    ...
    def handle_buy(self, msg: Message, client_address: tuple):
        try:
            ...
            with self.reservation_lock:
                ...
                if not reservation:
                    logging.warning(f"No reservation found for RQ# {msg.rq_number}")
                    return
                ...
                # Proceed to finalize purchase over TCP
                threading.Thread(target=self.finalize_purchase, args=(reservation, msg.rq_number), daemon=True).start()
    ...
    def finalize_purchase(self, reservation: ItemReservation, rq_number: str):
        try:
            ...
            buyer_address = (buyer_info['ip'], buyer_info['tcp_port'])
            seller_address = (seller_info['ip'], seller_info['tcp_port'])

            # Accept connections from buyer and seller
            # For simplicity, we assume the buyer and seller will connect to the server
            # The server listens for connections and handles the transaction

            # Wait for connections
            buyer_conn, buyer_addr = self.tcp_socket.accept()
            seller_conn, seller_addr = self.tcp_socket.accept()
            logging.info(f"Accepted TCP connections from buyer and seller")
            ...
            buyer_conn.sendall(inform_req.encode())
            seller_conn.sendall(inform_req.encode())

            # Receive INFORM_RES from buyer and seller
            buyer_data = buyer_conn.recv(1024).decode()
            seller_data = seller_conn.recv(1024).decode()

            buyer_msg = self.message_handler.parse_message(buyer_data)
            seller_msg = self.message_handler.parse_message(seller_data)

            # Process payment (simulated)
            transaction_success = self.process_payment(buyer_msg, seller_msg, reservation.price)

            if transaction_success:
                # Send TRANSACTION_SUCCESS to buyer
                success_msg = self.message_handler.create_message(
                    MessageType.TRANSACTION_SUCCESS,
                    rq_number
                )
                buyer_conn.sendall(success_msg.encode())

                # Send SHIPPING_INFO to seller with buyer's name and address
                ...
                # Send TRANSACTION_FAILED to both buyer and seller
                fail_msg = self.message_handler.create_message(
                    MessageType.TRANSACTION_FAILED,
                    rq_number,
                    reason="Payment processing failed"
                )
                buyer_conn.sendall(fail_msg.encode())
                seller_conn.sendall(fail_msg.encode())
                logging.info("Transaction failed during payment processing")
